...
static void websocket_handler_pack(websocket_pack_t *pack)
{
    ...
    if (g_ws_cfg.enable && g_ws_cfg.port_type != WS_PORT_SHELL) {
        ...
        case WS_PORT_ZIGBEE:
        {
            ...
            //清空虚拟机,直接发送数据
            ws_vm_rx_clear();
            if (serial_helper_is_open(BOARD_ZIGBEE_UART)) {
                ucZigbeeMode(ZIGBEE_MSG_MODE_BROAD);
                serial_helper_send(BOARD_ZIGBEE_UART, (const void *)pack->data, pack->len);
        ...
        {
            //清空虚拟机,直接发送数据
            extern void ws_gprs_try_send(rt_uint16_t port, void *buffer, rt_size_t size);
            ws_vm_rx_clear();
            ws_gprs_try_send(g_ws_cfg.listen_port, (void *)pack->data, pack->len);
            break;
        }
        case WS_PORT_LORA:
            break;
