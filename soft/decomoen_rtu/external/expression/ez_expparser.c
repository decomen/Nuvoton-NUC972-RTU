...
static double _factor(struct ez_exp *e, ez_flag *flag)
{
    if (e->sym == T_RETURN) {
        ...
        if (e->sym >= T_SET_FIRST && e->sym <= T_SET_LAST) {
            e->number = _expression_set(n, e, flag);
        } else {
            e->number = e->R[n];
        }
        ...
        if (!EZ_CHECK(e) && (EZ_SKIP(flag) || EZ_RETURN(e))) {
            ...
            if (e->tokenizer.actual_string && e->tokenizer.actual_size > 0 && e->tokenizer.actual_size < 32) {
                char tmp[32] = {0};
                memcpy(tmp, e->tokenizer.actual_string, e->tokenizer.actual_size);
