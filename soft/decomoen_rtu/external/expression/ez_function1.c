...
{
    int index = (int)(number + 0.5) - 1;
    //printf("_diget : %d\n", index);
    return index >= 0 ? das_do_get_di_state(index) : -1;
}
...
static double _doget(double number)
{
    ...
    if (index < RELAYS_OUTPUT_NUM) {
        return index >= 0 ? das_do_get_do_state(DAS_DIDO_TYPE_RELAY, index) : -1;
    } else {
        return index >= 0 ? das_do_get_do_state(DAS_DIDO_TYPE_TTL, index - RELAYS_OUTPUT_NUM) : -1;
    }
}
