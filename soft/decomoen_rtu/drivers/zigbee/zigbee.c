...
#define ZIGBEE_SEND_BUFFER(_buffer,_n) do { \
        serial_helper_send(BOARD_ZIGBEE_UART, (const void *)_buffer, _n); \
    } while (0)
    ...
    if (bZigbeeInCmd() &&
        ...
        if (g_xfer_net_dst_uart_occ[BOARD_ZIGBEE_UART]) {
            if(ZGB_TM_GW == g_zigbee_cfg.tmode) {
                ...
                xfer_dst_serial_recv_buffer(BOARD_ZIGBEE_UART, (void *)s_zgb_rcv_queue.buf, s_zgb_rcv_queue.len);
            }
...
BOOL bZigbeeInit(BOOL uart_default)
{
    ...
    s_zgb_serial = g_serials[BOARD_ZIGBEE_UART];
    ...
    serial_helper_open(BOARD_ZIGBEE_UART);
...
static ssize_t __mb_zgb_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (req && req_length > 0) {
        return serial_helper_send(BOARD_ZIGBEE_UART, (const void *)req, (rt_size_t)req_length);
    }
    return -1;
}
