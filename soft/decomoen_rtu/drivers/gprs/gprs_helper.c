...
_start:
 ...
 //打开GPRS电源，判断当前接的是哪种模块
   if(vCheckCellNetType() != E_4G_EC20){
        rt_kprintf("未检测到4G模块\r\n");
        vGPRS_PowerDown();
        vGPRS_PowerUp();
        rt_thread_delay(RT_TICK_PER_SECOND / 10);
         __gprs_term_ctrl(1);
        rt_thread_delay(1.0 * RT_TICK_PER_SECOND);
        __gprs_term_ctrl(0);
        rt_kprintf("GPRS wait 20s check network type!\r\n");
...
_retry:
        ...
        while (!bAllInitOk) {
            ...
            if (g_gprs_work_cfg.eWMode != GPRS_WM_SHUTDOWN) {
                ...
                if(bGPRS_ATTest()) {
                    ...
                    if (bAllInitOk && (g_xCellNetType != E_4G_EC20) ) {
    ...
    if (bIsGprsInitOk) {
        ...
        if (!__gprs_wait_pppd_up(30)) {
            system("killall pppd");
            bIsGprsInitOk = RT_FALSE;
            if (shutdown_if_init_error) {
                g_gprs_work_cfg.eWMode = GPRS_WM_SHUTDOWN;
            }
            vGPRS_PowerDown();
        } else {
            if (!g_net_cfg.dhcp) {
                das_do_add_default_route(g_net_cfg.gw);
            } else if (net_is_link()) {
                struct das_net_list_node net;
                memset(&net, 0, sizeof(net));
                das_do_get_net_info(DAS_NET_TYPE_ETH, 0, &net);
            } else {
                das_do_add_default_route("");
            }
        }
    } else {
        vGPRS_PowerDown();
