...
rt_bool_t cc_bjdc_open(rt_uint8_t index)
{
    ...
    RT_KERNEL_FREE(s_pCCBuffer[index]);
    s_pCCBuffer[index] = RT_KERNEL_CALLOC(CC_BJDC_BUF_SIZE);
    s_CCBufferPos[index] = 0;
    s_cc_reinit_flag[index] = RT_FALSE;
...
{
    s_cc_reinit_flag[index] = RT_TRUE;
}
...
{
    {
        rt_uint8_t *pBuffer = s_pCCBuffer[index];
...
//      relate->count = 0; relate->time = rt_tick_get();
        while (!s_cc_reinit_flag[index]) {
            ExtData_t *node = RT_NULL;
            rt_thddog_feed("");
            rt_enter_critical();
            {
                while (!s_cc_reinit_flag[index]) {
                    node = pVarManage_GetExtDataWithUpProto(node, PROTO_DEV_NET_TYPE(index), index, PROTO_CC_BJDC);
                    if (node) {
            ...
            if (RT_EOK == rt_mq_recv(s_CCDataQueue[index], &xData, sizeof(CC_BJDC_Data_t), 2 * RT_TICK_PER_SECOND)) {
                ...
                case CC_BJDC_PT_PERIOD_CFG:
                {
                    ...
                    if (period > 0) {
                        vVarManage_SetExtDataUpInterval(PROTO_DEV_NET_TYPE(index), index, period * 60);
                    }
                    break;
                }
        ...
        if (s_cc_reinit_flag[index]) goto _START;
    }
...
//  rt_kprintf("-------------------------\n");
    if (xml != RT_NULL) {
        ...
        if (buf != RT_NULL) {
            ...
            cc_net_send(index, (const rt_uint8_t *)buf, ulLen + 14);
            rt_free(buf);
