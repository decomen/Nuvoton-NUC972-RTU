    ...
    *s_dm101_cfg_data[index] = c_dm101_default_cfg;
    {
        ...
        if (ini) {
            ...
            s_dm101_cfg_data[index]->crypt = ini_getint(ini, "common", "crypt", 1);
...
_START:
        ...
        net_waitconnect(index);
        {
            ...
            showtime();
            while (!s_cc_reinit_flag[index] && net_isconnect(index)) {
                ...
                if(g_xDm101Status[index].eInitStatus == E_DM101_REPORT_DATA) {
                    ...
                    {
                        while (!s_cc_reinit_flag[index] && net_isconnect(index)) {
                            node = pVarManage_GetExtDataWithUpProto(node, PROTO_DEV_NET_TYPE(index), index, PROTO_DM101);
                            if (node) {
...

                    {
                        ...
                        if (up_interval == 0) up_interval = 10;
                                        ...
                                        if (node->xUp.bEnable) {
                                            if (up_interval > 0 && rt_tick_get() - node->xUp.xAvgUp.time >= rt_tick_from_millisecond(up_interval * 1000)) {
                                                ...
                                                if (node->xUp.xAvgUp.count > 0) {
                                                    ...
                                                    pkg.data[pkg.data_cnt].status = 1;
            ...
            if (s_cc_reinit_flag[index] || !net_isconnect(index)) goto _START;
        }
    }
