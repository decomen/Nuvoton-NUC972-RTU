...
{
    switch(index) {
    case 0: return "/dev/ttyS9";
    case 1: return "/dev/ttyS3";
    case 2: return "/dev/ttyS2";
    case 3: return "/dev/ttyS1";
    case 4: return "/dev/ttyS5";
    case 5: {
...
int das_do_add_default_route(const char *wired_gw)
{
    char cmd[128] = {0};

    rt_kprintf("deleting routers\n");
    {
    ...
    case NET_ADAPTER_WIRED: {
        rt_kprintf("add eth0 routers\n");
        {
            ...
            sprintf(cmd, "route add default gw %s dev %s", wired_gw, das_do_get_net_driver_name(DAS_NET_TYPE_ETH, 0));
            system(cmd);
            printf("cmd: %s\r\n",cmd);
        }
        break;
    ...
    case NET_ADAPTER_WIRELESS: {
        ...
        {
            memset(cmd, 0, sizeof(cmd));
            sprintf(cmd, "route add default dev %s", das_do_get_net_driver_name(DAS_NET_TYPE_LTE, 0));
            system(cmd);
            printf("cmd: %s\r\n",cmd);
        }
        break;
...
int das_do_set_net_info(const struct das_net_list_node *node)
{
    ...
    if (interface_name) {
        ...
        if (eth_info.DHCP == 0 && node->DHCP) {
            ...
            das_do_add_default_route(node->GATEWAY);
        ...
        /*memset(cmd, 0, sizeof(cmd));
        if (strcmp(node->STATUS, "UP") == 0) {
            sprintf(cmd, "ifconfig %s up", interface_name);
        } else if (strcmp(node->STATUS, "DOWN") == 0) {
