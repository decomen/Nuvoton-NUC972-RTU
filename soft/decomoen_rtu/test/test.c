...
static void prvTestUart2ReciveTask( void* parameter )
{
    int i = 0;
    mdBYTE buf[512] = {0};
    rt_kprintf("prvTestUart2ReciveTask \n");

    while(1){
           // if(test_serial_helper_select(g_xTestUartInfo[HW_UART2].fd, 1000) > 0){
                  int n = read(g_xTestUartInfo[HW_UART2].fd, buf, 512);
                  for(int j = 0; j < n ; j++){
                    bQueueWrite(g_xTestUartInfo[HW_UART2].queue,buf[j]);
                    //printf("%02x\n",buf[j]);
                  }
                   usleep(1000);
                 // printf("===========num: %d, fd: %d\n",HW_UART2,g_xTestUartInfo[HW_UART2].fd);
            //}
        //
   }
}

static void prvTestUart3ReciveTask( void* parameter )
{
    int i = 0;
    mdBYTE buf[512] = {0};
    rt_kprintf("prvTestUart3ReciveTask \n");

    while(1){
          //  if(test_serial_helper_select(g_xTestUartInfo[HW_UART3].fd, 1000) > 0){
                  int n = read(g_xTestUartInfo[HW_UART3].fd, buf, 512);
                  for(int j = 0; j < n ; j++){
                    bQueueWrite(g_xTestUartInfo[HW_UART3].queue,buf[j]);
                    //printf("%02x\n",buf[j]);
                  }
                  usleep(1000);
               //   printf("============num: %d, fd: %d\n",HW_UART3,g_xTestUartInfo[HW_UART3].fd);
           // }
...
void vTestLedToggle()
{
    ...
    if(led_flag == 0){
        ...
        iodata.dir = 1;
        iodata.val = 0;
        das_do_io_ctrl(TEST_LED, &iodata);
...
int vIsTestModeIo(void)
{
    ...
    iodata.dir = 0;
    iodata.val = -1;
    das_do_io_ctrl(TEST_ATE, &iodata);

    s_io_t iodata1;
    iodata1.gpio = 0;
    iodata1.dir = 0;
    iodata1.val = -1;
    das_do_io_ctrl(TEST_MODE, &iodata1);
    
    printf("key: %d , iodata1.val: %d\r\n",iodata.val,iodata1.val);
    
    return ((iodata.val==0) || (iodata1.val==0));
}
...
static void prvTestTask( void* parameter )
{
   ...
   /* int fd = -1;
    ...
    if( RT_NULL == g_TestUart2Thread ) {
        g_TestUart2Thread = rt_thread_create( "uart2_recive", prvTestUart2ReciveTask, RT_NULL, 4096, 20, 20 );
        
        if( g_TestUart2Thread ) {
            rt_thread_startup( g_TestUart2Thread );
        }
    }

    if( RT_NULL == g_TestUart3Thread ) {
        g_TestUart3Thread = rt_thread_create( "uart3_recive", prvTestUart3ReciveTask, RT_NULL, 4096, 20, 20 );
        
        if( g_TestUart3Thread ) {
            rt_thread_startup( g_TestUart3Thread );
        }
    }

    

     if( RT_NULL == g_TestRelaysThread ) {
        g_TestRelaysThread = rt_thread_create( "Relays_recive", prvTestRelaysTask, RT_NULL, 4096, 20, 20 );
        
        if( g_TestRelaysThread ) {
            rt_thread_startup( g_TestRelaysThread );
        }
    }
	...
	switch(rtype){
		case UART_GET_0:{
			if(cType == UART_RECIVE){
				byte buf[256] = {0};
				if(ReciveUartData(HW_UART0, buf, sizeof(buf), 200, 200, 500) > 0 ){
                    int i;
					for(i = 0; i < 20;i++){
                       // printf("%02x ",buf[i]);
						if(buf[i] != i) break;
		...
		case UART_GET_1:{
			if(cType == UART_RECIVE){
				byte buf[256] = {0};
				if(ReciveUartData(HW_UART1, buf, sizeof(buf), 200, 200, 500) > 0){
                    int i;
					for( i = 0; i < 20; i++){
                        //printf("%02x ",buf[i]);
						if(buf[i] != i) break;
