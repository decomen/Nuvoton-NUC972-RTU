...
rt_err_t net_cfg_readd_dns(void)
{
    if (g_net_cfg.dhcp) {
        ...
        if (g_net_cfg.dns[0] && inet_aton(g_net_cfg.dns, &peer.sin_addr) && !das_do_check_dns(g_net_cfg.dns)) {
            sprintf(cmd, "echo 'nameserver %s # %s' >> /etc/resolv.conf", g_net_cfg.dns, interface_name);
            my_system(cmd);
        }/* else if (!das_do_check_dns("8.8.8.8")) {
            sprintf(cmd, "echo 'nameserver %s # %s' >> /etc/resolv.conf", "8.8.8.8", interface_name);
            my_system(cmd);
        }*/
        my_system("grep ppp0 /tmp/resolv.prev >> /etc/resolv.conf");
    }
    return -RT_EOK;
}
