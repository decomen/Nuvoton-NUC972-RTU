...
void SaveAdcCalInfoToFs(void)
{
    ...
    usCheck = das_crc16((uint8_t *)&gCalEntryBak, sizeof(gCalEntryBak));
    memcpy(&buffer[4 + sizeof(gCalEntryBak)], &usCheck, 2);
    block_erase(ADC_INFO_OFFSET);
    block_write("/dev/mtd4", ADC_INFO_OFFSET, buffer, sizeof(buffer));
...
static void ReadAdcCalInfoFromFs(void)
{
    ...
    block_read("/dev/mtd4", ADC_INFO_OFFSET, buffer, sizeof(buffer));

    memcpy(&magic, (void const *)buffer, 4 );
    if (CFG_MAGIC == magic) {
        memcpy(&gCalEntry, (void const *)&buffer[4], sizeof(gCalEntry) );
        memcpy(&usCheck, (void const *)&buffer[4 + sizeof(gCalEntry)], 2 );
        if (das_crc16((uint8_t *)&gCalEntry, sizeof(gCalEntry)) != usCheck) {
            SetAdcCalCfgDefault();
        }
    } else {
        SetAdcCalCfgDefault();
