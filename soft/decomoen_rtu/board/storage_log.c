...
#define _LOG_DEL_NUM                    (1000)
// 重要 log 暂存 10000
#define _LOG_LIMIT                      (100000)
    ...
    if (pCfg) {
        ...
        if (s_log_dbinit) {
            rt_mutex_take(&s_log_mutex, RT_WAITING_FOREVER);
            {
                sqlite3_stmt *stmt = RT_NULL;
                char sql[256] = "";
                rt_sprintf(sql, "SELECT id,time,level,log FROM "TABLE_LOG_DATA" WHERE time>=%u AND time<=%u ORDER BY time ASC", (unsigned int)start_time, (unsigned int)end_time);
